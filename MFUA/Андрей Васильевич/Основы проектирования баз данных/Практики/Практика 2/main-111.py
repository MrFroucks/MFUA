...
db = sq.connect(r"E:\Documents\MFUA\MFUA\Андрей Васильевич\Основы проектирования баз данных\Практики\Практика 2\database2.db")
c = db.cursor()
...
def get_users_with_important_tasks(users_with_tasks):
    q = ['?' for j in users_with_tasks]
    q = ', '.join(q)
    users_with_important_tasks = c.execute(f'SELECT Пользователь FROM RD1_Задания WHERE Пользователь IN ({q}) AND Срочное = 1', users_with_tasks).fetchall()
    users_with_important_tasks = [j[0] for j in users_with_important_tasks]
    users_with_important_tasks = list(map(str, users_with_important_tasks))
    return users_with_important_tasks 
...
def MAIN():
    ...
    users_with_important_tasks = get_users_with_important_tasks(users_with_tasks)
    print(f'Пользователи из Москвы со срочными заданиями: {', '.join(users_with_important_tasks)}')
