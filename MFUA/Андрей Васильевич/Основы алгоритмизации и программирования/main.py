...
def convert_dec(num):
    ...
    one_num, two_num = num.split('.')
    dec_one_num = int(one_num, s)
    c = 0
    dec_two_num = ''
    for i in two_num:
        c += 1
        num_post = int(i)*2**-c
        dec_two_num = str(num_post)
    if float(dec_two_num) >= 1:
        add_num, no = str(dec_two_num).split('.')
        dec_one_num += int(add_num)  
        return f'{dec_one_num}.{dec_two_num}'
    else:
        return f'{dec_one_num}.{dec_two_num}'
